...
#                                                                                                              #
# Dendritic Twin Delayed Deep Deterministic Policy Gradient (TD3) Implementation                               #
################################################################################################################
...
class Critic(nn.Module):
    ...
    # NOTE: With PAI, it's easier to make a single state_action tensor in the main loop
...
class TD3:
    def __init__(
        ...
        self.pai_model = pai_model

        # Perforated AI Optimizer + Scheduler
        GPA.pai_tracker.set_optimizer(torch.optim.Adam)
        GPA.pai_tracker.set_scheduler(torch.optim.lr_scheduler.ReduceLROnPlateau)

        optimArgs = {
            "params": [
                {"params": list(pai_model.critic.parameters()), "lr": lr},
        ...
        self.target_actor   = UPA.deep_copy_pai(self.actor)
        self.target_critic  = UPA.deep_copy_pai(self.critic)
    ...
    def set_actor_training(self, mode):
        for param in self.pai_model.actor.parameters():
            param.requires_grad = mode

    def set_critic_training(self, mode):
        for param in self.pai_model.critic.parameters():
            param.requires_grad = mode
    ...
    def step(self, transition):
        ...
        self.set_actor_training(False)

        self.critic_opt.zero_grad()
        q_loss.backward()
        self.critic_opt.step()

        self.set_actor_training(True)
        
        # (Delayed) Actor Update
        if do_actor_update:
            self.set_critic_training(False)
...
def run(args):
    ...
    while (not (pai_training_complete) and (total_steps < args.total_steps)):
        ...
        if args.eval_every > 0 and total_steps % args.eval_every == 0:
            ...
            if pai_training_complete:
                ...
                optimArgs = {
                    "params": [
                        {"params": list(pai_model.critic.parameters()), "lr": agent.lr},
...
if __name__ == '__main__':
    ...
    # Wandb
    parser.add_argument("--wandb_project", type=str, default="dendrite-td3-dmcontrol",
